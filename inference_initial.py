...
def main():
    ...
    if(args.pretrained_model == "dauhst"):
       model = DAUHST(num_iterations=9, pretrained_model_path=args.ckpt).to("cuda:3")
    if(args.pretrained_model == "padut"):
       model = PADUT(in_c=28,  n_feat=28,  nums_stages=2, n_depth=3, pretrained_model_path=args.ckpt).to("cuda:3")
    if(args.pretrained_model == "mst"):
       model = MST(dim=28, stage=2, num_blocks=[4,7,5], pretrained_model_path=args.ckpt).to("cuda:3")
    if(args.pretrained_model == "lambda"):
       model = Lambda_Net(out_ch=28, pretrained_model_path=args.ckpt).to("cuda:3")
    if(args.pretrained_model == "ssr"):
       model = SSRNet(stage=9, pretrained_model_path=args.ckpt).to("cuda:3")
    if(args.pretrained_model == "dpu"):
       model = DPUNet(stage=9, pretrained_model_path=args.ckpt).to("cuda:3")
    if(args.pretrained_model == "hdnet"):
       model = HDNet(pretrained_model_path=args.ckpt).to("cuda:3")
    resshift_sampler.inference(args.in_path, args.out_path, bs=1, noise_repeat=False, one_step=False, model=model, pretrained_model=args.pretrained_model, dataset=args.dataset)
    import evaluate
