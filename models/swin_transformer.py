...
def adjust_input_shape(x, window_size):
    B, H, W, C = x.shape
...
#def window_partition_test(x, window_size):
#    B, H, W, C = x.shape
#    if H % window_size != 0 or W % window_size != 0:
#        raise ValueError(f"Input dimensions (H={H}, W={W}) must be divisible by window_size ({window_size})")
#    
#    print(f"Input shape: {x.shape}, Window size: {window_size}")
#    
#    target_shape = (B, C, H // window_size, window_size, W // window_size, window_size)
#    print(f"Target shape: {target_shape}, Total elements: {np.prod(target_shape)}, Input elements: {x.numel()}")
#    
#    if np.prod(target_shape) != x.numel():
#        raise RuntimeError(f"Shape {target_shape} is invalid for input of size {x.numel()}")
#    
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        if mask is not None:
            nW = mask.shape[0]
            #attn = adjust_input_shape(attn, self.window_size[0])
...
class SwinTransformerBlock(nn.Module):
    ...
    def calculate_mask(self, x_size):
        ...
        # 在模型的前处理步骤中调整输入形状
        #img_mask = adjust_input_shape(img_mask, self.window_size)

        # 在 window_partition 函数中查看调试信息
        #x_windows = window_partition_test(shifted_x, self.window_size)  # (NW*B) x Ws x Ws x C
    ...
    def forward(self, x):
        ...
        '''
        B, C, Ph, Pw = x.shape
        if Ph % self.window_size != 0 or Pw % self.window_size != 0:
           pad_h = (self.window_size - Ph % self.window_size) % self.window_size
           pad_w = (self.window_size - Pw % self.window_size) % self.window_size
           x = F.pad(x, (0, pad_w, 0, pad_h))
           B, C, Ph, Pw = x.shape
        x_size = (Ph, Pw)
